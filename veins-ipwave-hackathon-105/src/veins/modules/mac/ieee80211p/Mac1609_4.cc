...
//            Ipv6Address destAddr = pkt->getTag<L3AddressReq>()->getDestAddress().toIpv6();
//            std::cout << "mac 1609, test pkt tag: "<< destAddr <<endl;

            if (destMac != inet::MacAddress::UNSPECIFIED_ADDRESS) { // -1 to unspecified
//                std::cout << "MAC DEST is not unspecified 111"<< endl;
//                std::cout << destMac << endl;
                mac->setDestAddr(destMac);
            } else {
//                std::cout << "MAC DEST ADDRESS IS BROADCAST"<< endl;
		...
		else if (wsm = dynamic_cast <WSM_ipwave*>(pktToSend))
		{
            ...
            if (destMac != inet::MacAddress::UNSPECIFIED_ADDRESS) { // -1 to unspecified
                ...
                std::cout << "MAC DEST ADDRESS IS BROADCAST222"<< endl;
                mac->setDestAddr(inet::MacAddress::BROADCAST_ADDRESS);
            }
