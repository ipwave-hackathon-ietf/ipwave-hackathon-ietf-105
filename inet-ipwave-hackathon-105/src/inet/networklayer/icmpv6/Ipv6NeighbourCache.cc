...
//XIANG ZHONG 19.05.21
Ipv6NeighbourCache::Neighbour *Ipv6NeighbourCache::replaceNeighbour(const Ipv6Address& globalAddr, const Ipv6Address& linkLocalAddr, int interfaceID, MacAddress macAddress, simtime_t registLifetime, const Ipv6Address& relayAddr)
{
    Key key(globalAddr, interfaceID);
    ASSERT(neighbourMap.find(key) == neighbourMap.end());    // entry must not exist yet
    Neighbour& nbor = neighbourMap[key];
//    nbor.registLifeTime = simTime()+registLifetime;
    nbor.registLifeTime = registLifetime;
    nbor.nceKey = lookupKeyAddr(key);
    nbor.macAddress = macAddress;
    nbor.isRouter = false;
    nbor.isHomeAgent = false;
    nbor.reachabilityState = TENTATIVE;
    nbor.relayAddr = relayAddr;
    nbor.reachabilityExpires = registLifetime;
    nbor.linkLocalAddr = linkLocalAddr;
    return &nbor;
...
/*
Ipv6NeighbourCache::Neighbour *Ipv6NeighbourCache::replaceNeighbourRoutingTable(const Ipv6Address& linkLocalAddr,
        const Ipv6Address& globalAddr, int interfaceID)
{
    Key key(linkLocalAddr, interfaceID);
    ASSERT(neighbourMap.find(key) == neighbourMap.end());    // entry must not exist yet
    Neighbour& nbor = neighbourMap[key];
    nbor.nceKey = lookupKeyAddr(key);
    nbor.nceKey->address = globalAddr;
    return &nbor;
