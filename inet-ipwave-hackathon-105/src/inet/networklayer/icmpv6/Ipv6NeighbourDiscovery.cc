...
//    XIANG ZHONG test relay neighborCache
    ...
    //XIANG ZHONG 19.06.03
    else if (isRelay) {
        nextHopAddr = nce->relayAddr;
        outIfID = nce->nceKey->interfaceID;
//        std::cout <<"set relay address:" <<nce->relayAddr <<" as next hop address."<<endl;
    }
    else {
        ...
        const Ipv6Address& srcAddr, int interfaceId)
{
    ...
    //test mac address 19.05.26
    InterfaceEntry *ie = ift->getInterfaceById(interfaceId);
//    delete msg->removeTagIfPresent<MacAddressReq>();
    //4
    ...
    //        std::cout<<"set mac dest addr to: "<<packet->getTag<MacAddressReq>()->getDestAddress()<<endl;
//            EV_INFO<<"set mac dest addr to: "<<msg->getTag<MacAddressReq>()->getDestAddress()<<endl;
    }


//    if (msg->getTag<MacAddressReq>()!=nullptr)
//        std::cout << "send packet to ipv6 module, mac address is:"<<msg->getTag<MacAddressReq>()->getSrcAddress() <<endl;
    msg->addTagIfAbsent<HopLimitReq>()->setHopLimit(255);
    //XIANG ZHONG
//    simtime_t delay = uniform(0.001,0.02);
    simtime_t delay = uniform(0.00001,0.0002); //1us to 20us
//    send(msg, "ipv6Out");
...
//            scheduleAt(30+interval, rtrDisMsg);
        }
...
//    std::cout<<"retans: " << ie->ipv6Data()->getRetransTimer() << endl;
        ...
        // update: added uniform(0, IPv6_MAX_RTR_SOLICITATION_DELAY) to account for joining the solicited-node multicast
        // group which is delay up to one 1 second (RFC 4862, 5.4.2) - 16.01.08, CB
        scheduleAt(simTime() + ie->ipv6Data()->getRetransTimer() + uniform(0, IPv6_MAX_RTR_SOLICITATION_DELAY), msg2);
    #endif /* WITH_xMIPv6 */
...
//    simtime_t interval = uniform(ie->ipv6Data()->getMinRtrAdvInterval(), ie->ipv6Data()->getMaxRtrAdvInterval());
    ...
    if (addrRegistEntry!=nullptr && addrRegistEntry->numNSSent < 2) {
        ...
        if (neighbourCache.getDefaultRouterList().begin() != neighbourCache.getDefaultRouterList().end()) {
            ...
            Neighbour& nce = *it;
            //XIANG ZHONG 19.07.11
//            destAddr = nce.globalAddr;
...
void Ipv6NeighbourDiscovery::processMultihopDadTimeout(cMessage *msg)
{
    ...
    std::cout << "dupAddrDetectTrans is: " << ie->ipv6Data()->getDupAddrDetectTransmits() << endl;

    //XIANG ZHONG 19.06.25
...
void Ipv6NeighbourDiscovery::initiateRouterDiscovery(cMessage *msg)
{
    ...
    //XIANG ZHONG 19.06.28 simulation test
...
//XIANG ZHONG 19.06.28
void Ipv6NeighbourDiscovery::processNdTimeout(cMessage *msg)
{
    InterfaceEntry *ie = (InterfaceEntry *)msg->getContextPointer();
    if (neighbourCache.getDefaultRouterList().begin() == neighbourCache.getDefaultRouterList().end())
    {
        std::cout <<"Restart Neighbor Discovery!"<<endl;
...
void Ipv6NeighbourDiscovery::processRdTimeout(cMessage *msg)
{
    ...
    //XIANG ZHONG simulation test 19.06.25
...
//        if (rdEntry->numRSSent == ie->ipv6Data()->_getMaxRtrSolicitations())
        ...
        //XIANG ZHONG 19.06.28 simulation test, reduce the retransmision interval
        if (rdEntry->numRSSent == 1)
...
//            scheduleAt(simTime() + ie->ipv6Data()->_getRtrSolicitationInterval(), msg);
        ...
        //XIANG ZHONG
        //Assumption: if no router on-link, look for vehicle neighbors
        Ipv6Address tentativeAddr = ie->ipv6Data()->getLinkLocalAddress();
        createAndSendNsPacket(tentativeAddr, Ipv6Address::ALL_NODES_2,
                    tentativeAddr, ie);
...
//            hopDistance = prefixInfo.getDistance();
        ...
        //XIANG ZHONG 19.05.26
        numRAsent++;
//        std::cout <<"numRAsent: "<<numRAsent <<endl;
    }
}
...
//    std::cout <<"NS Target Address: " << nsTargetAddr <<endl;
    ...
    //XIANG ZHONG  19.06.02
    if (neighbourCache.getDefaultRouterList().begin() != neighbourCache.getDefaultRouterList().end()) {
        Neighbour& router = *neighbourCache.getDefaultRouterList().begin();
        if (ns->isDadMsg() /*&& !router.relayAddr.isUnspecified()*/)
        {
            ns->setIsRelay(true);
...
void Ipv6NeighbourDiscovery::processNsPacket(Packet *packet, const Ipv6NeighbourSolicitation *ns)
{
    ...
    else if (!rt6->isRouter() && entry == nullptr && neighbourCache.getDefaultRouterList().begin() != neighbourCache.getDefaultRouterList().end()){
...
//            destAddr = dac->getRegisteredAddress();
            //XIANG ZHONG 19.05.25
            destAddr = neighbourEntry->linkLocalAddr;

//        std::cout << "na destAddr is: " <<destAddr<< endl;
...
void Ipv6NeighbourDiscovery::forwardNDMessageForNeighbour(Packet *packet, const Ipv6Address& destAddr, InterfaceEntry *ie)
{
    ...
    //XIANG ZHONG 19.07.07
//    Ipv6Address dgSrcAddr = ie->ipv6Data()->getLinkLocalAddress();
...
//            neighbourCache.addRelayNeighbour(nsL3SrcAddr, ns->getLinkLocalAddress(),ifID, nsMacAddr ,
//                    ns->getARO().getRegistLifeTime(),relayAddr);
...
//            naDestAddr = neighbourEntry->relayAddr;
    ...
    EV_INFO<<"NA address is: "<< naDestAddr <<endl;
//    std::cout <<"Send solicited NA, dest addr is: " <<naDestAddr <<endl;
...
//        std::cout << "Set router address in NA, router address is: "<< destAddr <<endl;
...
void Ipv6NeighbourDiscovery::processNaPacket(Packet *packet, const Ipv6NeighbourAdvertisement *na)
{
    ...
    //XIANG ZHONG 19.06.11 NeighborRoute
...
//                        simTime() + entry.preferredLifetime, entry.hFlag);
        ie->ipv6Data()->assignAddress(naTargetAddr, false, simTime(),
                        simTime());
        dadFinishTime = simTime()-dadStartTime;
        std::cout  <<ie->ipv6Data()->getLinkLocalAddress() <<": " << dadFinishTime << endl;
...
//            std::cout <<ie->ipv6Data()->getLinkLocalAddress()<<" NA prefix distance: " << hopDistance <<", smallest hop: "<<neighborHopDistance << endl;
            //XIANG ZHONG 19.05.20
            Ipv6Address destAddr = packet->getTag<L3AddressInd>()->getSrcAddress().toIpv6();
...
// not used yet
/*
void Ipv6NeighbourDiscovery::addNeighborAsRouter(Packet *packet,InterfaceEntry *ie, const Ipv6NeighbourAdvertisement *na)
{
    Ipv6Address pktSrcAddr = packet->getTag<L3AddressInd>()->getSrcAddress().toIpv6();
    //XIANG ZHONG 19.05.26
//    neighbourCache.addRouter(pktSrcAddr, ie->getInterfaceId(),
    neighbourCache.addRouterWithGlobalAddr(na->getRouterAddress(), ie->getInterfaceId(),
            na->getSourceLinkLayerAddress(), simTime()+200, na->getRouterAddress());

    Neighbour *router = neighbourCache.lookup(na->getRouterAddress(), ie->getInterfaceId());
    if (router!=nullptr) {
        router->relayAddr = pktSrcAddr;
        router->hopDistance = neighborHopDistance;
//        std::cout << "add relay address to default router" << endl;
    }
}
*/
...
//    std::cout<<"prefix info, distance: "<<prefixInfo.getDistance() <<endl;
